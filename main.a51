...
;au debut les roues sont droites et le moteur est a l'arr?
...
ledmC  			bit     p1.0   ; indique ? en marche (led allumee si p1.0 = 0)
...
vd1h			equ		0fah		;(65536-1500)			   vd1=vm1
vd1l			equ		24h			;
...
vd2l			equ		7fh		; la direction		              adresse
vd2h			equ		7eh
...
tr0a0: 
				cjne	r0,#0,tr0a1		;direction  if r0 = 0
				mov		vtl0,vd2l
				mov		vth0,vd2h
				mov		r0,#1
				setb	dir
...
tr0a1:  
				cjne	r0,#1,tr0a2		;reste de la direction 	   if r0 = 1
       			mov		vtl0,vdr2l
        		mov		vth0,vdr2h
        		mov		r0,#2
       			clr    	dir
...
tr0a2:  
				cjne	r0,#2,tr0a3		;moteur     if r0 = 2
       			mov		vtl0,vm2l
        		mov		vth0,vm2h
        		mov		r0,#3
				setb	mot
...
tr0a3:  
				mov		vtl0,vmr2l		;reste du moteur	else
				mov		vth0,vmr2h
				clr		mot
				mov		r0,#0
...
relancet0:
				...
				addc	a,th0    ; pour tenir compte du d?ordement
...
;augmentation de la duree moteur selon ****R7****
sousmot: 								 
				...
				mov		vm2l,a
				mov		a,vm2h
				subb	a,#00h    ;toggle with the Carry
				mov		vm2h,a 
restmot2:
				clr		c
				mov		a,vmr2l
...
durecom:										 ; g?e r1 =  Nb de commandes soit r1*20ms 
				jnb		finint,durecom
				clr		finint
...
debut:
				...
				mov		tl1,#0E6h	; pour le d?marage.
				
				;Initialisation Serial 
				mov		scon,#52h	; mode 1 10 bits, ren=1,ti=1
				; lancement timer1 pour diviser fqz (baudrate)
				setb	tr1
...
;			org	0030h
; pr?ablement les capteurs d?ectent que le blanc
;debut :
;			JB		acc.6,tournedroite
;			JB		acc.7,tournegauche
